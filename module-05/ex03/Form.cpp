...
{
	// If it's not signed, we throw a FormUnsignedException.
	if (!_signed)
		throw FormUnsignedException();
	// If the bureaucrat cannot execute because it's grade is too low, we throw
	// an exception.
	if (executor.getGrade() > _executeGrade)
		throw GradeTooLowException();
	// If all went well, we go to the form's own action function.
	this->_performAction();
}
